    ...
    if (slugs.length) {
      slugPredicate = slugs
        .map(slug => Object.entries(slug)
          .map(([locale, value]) => `${locale}="${this._escapeValues([value])}"`)
          .join(' or '))
        .join(' or ')
      slugPredicate = ` or slug(${slugPredicate})`
    }

    const skusPredicate = this._getSkusPredicate(skus)
    const predicate = `masterData(current(${skusPredicate}${slugPredicate}) `
      + `or staged(${skusPredicate}${slugPredicate}))`
