...
export default class ProductManager {
   ...
   */
  getAnonymizedProduct (product) {
    const timestamp = this._getTimestamp()

    // loop through fields which have to be unique
    for (const field of ['name', 'slug'])
      for (const lang in product[field]) // eslint-disable-line guard-for-in
        // add timestamp to values which has to all languages
        product[field][lang] += `-${timestamp}`

    if (product.key)
      product.key += `-${timestamp}`
    product.slug[constant.PRODUCT_ANONYMIZE_SLUG_KEY] = timestamp
