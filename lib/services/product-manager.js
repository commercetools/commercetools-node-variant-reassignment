...
export default class ProductManager {
   ...
   */
  async removeVariantsFromProduct (product, skus) {
    const actions = await this.getRemoveVariantsActionFromProduct(product, skus)
    if (actions === null)
      return null
  ...
  getRemoveVariantsActionFromProduct (product, skus) {
   ...
   */
  getAnonymizedProductDraft (product, productDraft) {
    const salt = this._getSalt()
    const oldAnonymizationId = product.slug[constant.PRODUCT_ANONYMIZE_SLUG_KEY]

    if (pU.hasMatchingSlugs([product.slug], productDraft.slug)) {
    ...
    if (product.key && _.isEqual(product.key, productDraft.key))
      if (oldAnonymizationId)
