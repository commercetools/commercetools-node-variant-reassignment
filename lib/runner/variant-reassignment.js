...
export default class VariantReassignment {
  ...
  _processProductDraft (productDraft) {
    ...
    const { matchingProducts: backupVariants, ctpProductToUpdate: variantsToProcess }
  ...
  _getRemovedVariants (productDraft, matchingProducts, ctpProductToUpdate) {
    ...
    const collectVariantsWithCondition = (product, condition) => {
      const currentVariants = product.masterData.current.variants
        .concat(product.masterData.current.masterVariant)
      const stagedVariants = product.masterData.staged.variants
    ...
    return {
      matchingProducts: matchingProductsVariants,
      ctpProductToUpdate: ctpProductToUpdateVariants
    }
  }
  ...
  _backupToCustomObject (currentProductDraft, backupProductDraft, variants) {
    throw new Error(`implement the method for ${variants}`)
  }
