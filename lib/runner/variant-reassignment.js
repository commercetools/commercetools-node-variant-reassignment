...
import pU from '../utils/product-utils'

export default class VariantReassignment {

  constructor (client, logger, errorCallback = this._defaultErrorCallback.bind(this),
               retainExistingData = []) {
  ...
  async execute (productDrafts, productTypeNameToTypeObj = {}) {
    ...
    this.statistics.badRequestSKUs.push(...badRequestSKUs)
    return {
      badRequestSKUs,
  ...
  async _handleUnfinishedTransactionsError (retryCount) {
  ...
  async _retryProcessingOfProductDraft (productDraft, products, retryCount) {
    ...
    const failedTransaction = transactions.find(({ value }) =>
      _.isEqual(value.newProductDraft.slug, productDraft.slug)
    )
  ...
  async _handleUnrecoverableError (error) {
    try {
      const transactions = await this.transactionService.getTransactions()
      error[constants.ERROR_CONTEXT]
        = `Unrecoverable error, will delete backup custom object: ${JSON.stringify(transactions)}`
      await Promise.map(transactions,
        transaction => this.transactionService.deleteTransaction(transaction.key),
        { concurrency: 3 })
    } catch (e) {
      log('Error when deleting backup custom objects', e)
    } finally {
  ...
  async _processProductDraft (productDraft, products, retryCount = 0) {
    try {
      log(
      ...
      const anonymizedProductDraft
        = this._createProductDraftWithRemovedVariants(ctpProductToUpdate, variantsToProcess, productDraft)
  ...
  async _createAndExecuteActions (productDraft, anonymizedProductDraft, backupVariants,
    ...
    if (anonymizedProductDraft) {
      await this._removeVariantsAndUpdateSlugFromCtpProductToUpdate(anonymizedProductDraft, ctpProductToUpdate, productDraft)
  ...
  _createProductDraftWithRemovedVariants (product, variantsToBackup, productDraft) {
    ...
    if (variantsToBackup.length > 0) {
      ...
      productDraftClone = this.productService.getAnonymizedProductDraft(productDraftClone, productDraft)
    }
  ...
  async _ensureProductCreation (productDraft) {
    ...
    if (!existingProducts.length) {
      ...
      if (productDraft.slug[constants.PRODUCT_ANONYMIZE_SLUG_KEY]) {
        this.statistics.anonymizedSlug.push(Object.values(productDraft.slug)[0])
  ...
  async _ensureSlugUniqueness (productDraft, matchingProducts) {
    ...
    await Promise.map(productsToAnonymize, async (product) => {
      await this.productService.anonymizeCtpProduct(product)
      this.statistics.anonymizedSlug.push(Object.values(product.masterData.staged.slug)[0])
    }, { concurrency: 3 })
  ...
  /**
   * @param backupProductDraft
   * @param ctpProductToUpdate
   * @param productDraft
   * @returns {Promise<void>}
   * @private
   */
  async _removeVariantsAndUpdateSlugFromCtpProductToUpdate (backupProductDraft, ctpProductToUpdate, productDraft) {
    const skusToRemove = this.productService.getProductDraftSkus(backupProductDraft)
    const actions = await this.productService.getRemoveVariantsActionFromProduct(ctpProductToUpdate, skusToRemove)
    if (_.isArray(actions)) {
      const ctpProductToUpdateSlugs = [ctpProductToUpdate.masterData.current.slug, ctpProductToUpdate.masterData.staged.slug]
      // update slug and key in case slug and key from ctpProductToUpdate is still
      // not updated to avoid conflicts with backupProductDraft (see example 19)
      if (!pU.hasMatchingSlugs(ctpProductToUpdateSlugs, productDraft.slug))
        actions.push({
          action: 'changeSlug',
      ...
      if (!_.isEqual(ctpProductToUpdate.key, productDraft.key))
        actions.push({
          action: 'setKey',
          slug: productDraft.key
        })
  ...
  async _createVariantsInCtpProductToUpdate (backupVariants, productDraft, ctpProductToUpdate) {
    ...
    log(`Updating ctpProductToUpdate with ${actions.length} addVariant actions`)
  ...
  async _removeVariantsFromMatchingProducts (backupVariants, matchingProducts) {
    ...
    log('Removing variants from matching products')
  ...
  _defaultErrorCallback (error) {
    ...
    this.logger.error(`Error during reassignment - skipping productDraft.`,
      util.inspect(errorObj, false, null)
    )
    return Promise.resolve()
  }

  _isUnrecoverableError (error) {
