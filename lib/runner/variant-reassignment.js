...
export default class VariantReassignment {
  ...
  async _handleUnrecoverableError (error) {
    ...
    try {
      ...
      debug('Error when deleting backup custom objects', e)
    } finally {
  ...
  async _processUnfinishedTransactions (transactions, retryCount = 0) {
    try {
      if (!transactions) {
        debug('Loading unfinished transactions')
  ...
  async _processProductDraft (productDraft, products, retryCount = 0) {
    try {
      ...
      debug(`Selected ctpProductToUpdate with id "${ctpProductToUpdate.id}"`)
  ...
  async _createVariantsInCtpProductToUpdate (backupVariants, productDraft, ctpProductToUpdate) {
    ...
    debug(`Updating ctpProductToUpdate with ${actions.length} addVariant actions`)
  ...
  async _removeVariantsFromMatchingProducts (backupVariants, matchingProducts) {
    ...
    debug('Removing variants from matching products')
