...
export default class VariantReassignment {
  ...
  async execute (productDrafts, productTypeNameToTypeObj = {}) {
    ...
    for (const productDraft of productDraftsForReassignment)
      try {
        ...
        failedSkus.push(...this.productService.getProductDraftSkus(productDraft))
        await this._handleUnrecoverableError(e)
      } finally {
    ...
    this.statistics.failedSkus.push(...failedSkus)
    return {
      failedSkus,
  ...
  async _handleUnfinishedTransactionsError (error, retryCount) {
  ...
  async _handleUnrecoverableError (error) {
    ...
    await this.errorCallback(error, this.logger)
    try {
      const transactions = await this.transactionService.getTransactions()
  ...
  async _processUnfinishedTransactions (transactions, retryCount = 0) {
    try {
      ...
      if (retryCount < constants.MAX_RETRIES)
        await this._handleUnfinishedTransactionsError(e, retryCount)
      else
        throw e
    }
  }
  ...
  async _processProductDraft (productDraft, products, retryCount = 0) {
    try {
      ...
      if (retryCount < constants.MAX_RETRIES)
        await this._handleProcessingError(e, productDraft, products, retryCount)
      else
        throw e
    }
  }
