...
export default class VariantReassignment {
  ...
  /**
   * In productDrafts from external systems, there's no productTypeId, but instead the key is used.
   * However, productTypeId is needed for reassignment. This method replaces the productTypeKey
  ...
  _selectMatchingProducts (productDraft, products) {
    ...
    if (products) {
      ...
      const matchingProductsBySku = this._selectMatchingProductsBySkus(products,
        productDraftSkus)
      return _.compact(_.uniqWith(matchingProductsBySku.concat(matchingProductsBySlug), _.isEqual))
    }
