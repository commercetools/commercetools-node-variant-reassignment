...
export default class VariantReassignment {
  ...
  async execute (productDrafts, productTypeNameToTypeObj = {}) {
    ...
    try {
      ...
      await this._handleProcessingError(e)
    }
    this.firstRun = false
    ...
    if (isReassignmentRequired) {
      for (const productDraft of productDraftsForReassignment)
        try {
          ...
          await this._handleProcessingError(e, productDraft, products)
        } finally {
   ...
   */
  async _handleProcessingError (error, productDraft = null, products = null) {
    console.log('get here _handleProcessingError ')
    ...
    try {
      if (productDraft === null || products === null)
