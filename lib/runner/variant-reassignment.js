...
export default class VariantReassignment {
  ...
  async _createAndExecuteActions (productDraft, anonymizedProductDraft, backupVariants,
                                  ctpProductToUpdate, transaction, matchingProducts) {
    const _ctpProductToUpdate = ctpProductToUpdate
    ...
    if (!matchingProducts) {
      ...
      if (!ctpProductToUpdate)
        this.logger.warn(
          'Reassignment has empty ctpProductToUpdate! ' +
          'Logging out original ctpProductToUpdate, matchingProducts and productToUpdateCandidate:',
          util.inspect(_ctpProductToUpdate, false, null),
          util.inspect(matchingProducts, false, null),
          util.inspect(productToUpdateCandidate, false, null)
        )
    }
