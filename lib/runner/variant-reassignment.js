...
export default class VariantReassignment {
  ...
  constructor (client, logger, errorCallback = this._defaultErrorCallback.bind(this),
    ...
    this.whiteListedLocales = whiteListedLocales
  ...
  async execute (productDrafts, productTypeNameToTypeObj = {}) {
    ...
    try {
      products = await this.productService.fetchProductsFromProductDrafts(
        productDrafts,
        this.whiteListedLocales
      )
    } catch (e) {
  ...
  _selectMatchingProducts (productDraft, products) {
    ...
    return this.productService.getProductsBySkusOrSlugs(
        productDraftSkus,
        productDraftSlugs,
        this.whiteListedLocales
    )
